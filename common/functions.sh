...
ui_print "*     Powered by Magisk（@聖小熊）     *"
ui_print "**************************************"
...
if ! $BOOTMODE; then
  ui_print "- 在 Recovery 模式中只支援解除安裝"
  ui_print "- 正在解除安裝模組！"
...
if [ "$(ls -A $MODPATH/common/addon/*/install.sh 2>/dev/null)" ]; then
  for i in $MODPATH/common/addon/*/install.sh; do
    ui_print "- 執行 $(echo $i | sed -r "s|$MODPATH/common/addon/(.*)/install.sh|\1|")..."
    ui_print "  implement $(echo $i | sed -r "s|$MODPATH/common/addon/(.*)/install.sh|\1|")..."
    ui_print " "
    . $i
  done
fi
...
### Install
ui_print "- 安裝程序將在 3 秒後開始..."
ui_print "  Setup will start in 3 seconds..."
sleep 3
...
ui_print " "
sleep 1
ui_print "✓ 設定權限完成，重啟開始享受徠卡相機！"
ui_print "  Set permissions completed, 
  restart to enjoy Leica camera!"
ui_print " "
